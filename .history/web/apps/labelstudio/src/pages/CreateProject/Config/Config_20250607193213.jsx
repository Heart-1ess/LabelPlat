...
const EmptyConfigPlaceholder = () => (
  <div className={configClass.elem("empty-config")}>
    <p>当前配置为空，需要配置来进行标注。</p>
    <p>
      可以从预设中创建项目或自己配置项目。标注配置基于XML并且可以
    </p>
  </div>
);
  ...
  return (
    <div className={configClass.elem("labels")}>
      <form className={configClass.elem("add-labels")} action="">
        <h4>{tagname === "Choices" ? "添加选择" : "添加标注名称"}</h4>
        <span>用换行符作为分隔以添加新的标注</span>
        <textarea
          name="labels"
          id=""
          cols="50"
      ...
      <div className={configClass.elem("current-labels")}>
        <h3>
          {tagname === "Choices" ? "选择" : "标注"} ({control.children.length})
        </h3>
        <ul>
  ...
  return (
    <>
      <Select
        ...
        label={
          <>
            {template.objects > 1 && `从${obj.getAttribute("name")}`}
            中使用{obj.tagName.toLowerCase()}
            {columns?.length > 0 && columns[0] !== DEFAULT_COLUMN && "列"}
          </>
        }
  ...
  return (
    <div className={configClass.elem("object")}>
      ...
      {template.objects.length > 1 && columns?.length > 0 && columns.length < template.objects.length && (
        <p className={configClass.elem("object-error")}>此模板需要更多数据</p>
      )}
      {columns?.length === 0 && (
    ...
    <div className={configClass}>
      <div className={configClass.elem("container")}>
        ...
        <header>
          ...
          <ToggleItems items={{ code: "代码", visual: "视图" }} active={configure} onSelect={onSelect} />
        </header>
