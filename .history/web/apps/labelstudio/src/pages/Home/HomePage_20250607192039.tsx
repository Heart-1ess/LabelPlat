    ...
    <main className="p-6">
      <div className="grid grid-cols-[minmax(0,1fr)_450px] gap-6">
        <section className="flex flex-col gap-6">
          ...
          >
            {isFetching ? (
              ...
              <div className="h-64 flex justify-center items-center">加载项目失败</div>
              ...
              <div className="flex flex-col justify-center items-center border border-primary-border-subtle bg-primary-emphasis-subtle rounded-lg h-64">
                ...
                <Heading size={2}>创建一个项目</Heading>
                <Sub>导入数据并设置标签界面以开始注释</Sub>
        ...
        <section className="flex flex-col gap-6">
          ...
          <SimpleCard title="其他资源" description="Learn, explore and get help" data-testid="resources-card">
            <ul>
              {resources.map((link) => {
                return (
                  <li key={link.title}>
          ...
          <div className="flex gap-2 items-center">
            <IconHumanSignal />
            <span className="text-neutral-content-subtle">人在环路的标注系统</span>
          </div>
        </section>
      </div>
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <div className="text-neutral-content-subtler text-sm">
            在{total}个任务中共完成了{finished}个，进度({total > 0 ? Math.round((finished / total) * 100) : 0}%)
          </div>
        </div>
