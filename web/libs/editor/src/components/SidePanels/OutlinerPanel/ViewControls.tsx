      ...
      switch (value) {
        case "manual":
          return {
            ...
            selectedLabel: isFF(FF_DEV_3873) ? "人工" : "人工分组",
        ...
        case "label":
          return {
            label: (
              <>
                <IconBoundingBox /> 按标注分组
              </>
            ),
            selectedLabel: isFF(FF_DEV_3873) ? "按标注" : "按标注分组",
...
const ToggleRegionsVisibilityButton = observer<FC<ToggleRegionsVisibilityButton>>(({ regions }) => {
  ...
  return (
    <Elem
      ...
      aria-label={isAllHidden ? "展示所有标注" : "隐藏所有标注"}
      icon={
        isAllHidden ? (
      ...
      tooltip={isAllHidden ? "展示所有标注" : "隐藏所有标注"}
      tooltipTheme="dark"
    />
  );
});
