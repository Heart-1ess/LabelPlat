  ...
  settings: {
    placeLabelsLeft: {
      ...
      param: ($control, value) => {
        ...
        const inline = ["顶部", "底部"].includes(value);
        const reversed = ["顶部", "左部"].includes(value);
      ...
      value: ($control) => {
        ...
        if (!direction) {
          ...
          return position & Node.DOCUMENT_POSITION_FOLLOWING ? "顶部" : "底部";
        }
        if (direction[1] === "column") return direction[2] ? "顶部" : "底部";
        return direction[2] ? "左部" : "右部";
      },
    },
    filter: {
      title: "为长标签列表添加过滤器",
