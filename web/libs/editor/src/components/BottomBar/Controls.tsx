...
export const EMPTY_SUBMIT_TOOLTIP = "本项目拒绝空注释";
  ...
  observer(({ store, history, annotation }) => {
    ...
    if (buttonsReplacement) {
      ...
      if (userGenerate || (store.explore && !userGenerate && store.hasInterface("submit"))) {
        const title = submitDisabled ? EMPTY_SUBMIT_TOOLTIP : "保存：[ Ctrl+Enter ]";
      ...
      if ((userGenerate && sentUserGenerate) || (!userGenerate && store.hasInterface("update"))) {
        ...
        const button = (
          <ButtonTooltip key="update" title={noChanges ? "未做任何更改" : "更新此任务：[ Ctrl+Enter ]"}>
            <Button
              aria-label="submit"
              name="submit"
